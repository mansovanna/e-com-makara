...
const handleSubmit = async () => {
  if (!appStore.validateForm()) return
  ...
  await appStore.createProduct(formData)
}
</script>
  ...
  >
    <div @click.stop class="w-full max-w-2xl bg-white rounded-xl p-8 max-h-[90vh] overflow-y-auto">
      ...
      <div class="w-full flex justify-between items-center gap-3 mb-6">
        <h1 class="text-3xl text-orange-500 font-semibold">
          {{ appStore.formData.id ? 'Edit Product' : 'Create Product' }}
        </h1>
        <button
          @click="closeModal"
      ...
      <form @submit.prevent="handleSubmit" class="space-y-5">
        ...
        <div class="flex gap-3 justify-end pt-4 border-t border-slate-200">
          ...
          >
            {{ appStore.formData.id ? 'Update Product' : 'Create Product' }}
          </button>
        </div>
      </form>
    </div>
